...
#[command(
    name = "thther",
    version,
    about = "Persistent remote terminal over SSH+TCP"
)]
struct Cli {
    ...
    /// SSH port (1-65535); overrides ~/.ssh/config for this connection.
    #[arg(short = 'p', long, global = true, value_name = "PORT")]
    ssh_port: Option<NonZeroU16>,
...
async fn dispatch(cli: Cli) -> Result<()> {
    ...
    match cli.cmd {
        ...
        // --- client modes ---
        Some(Cmd::Attach { id }) => {
            client_attach(load_client_config(target, port, ssh_port)?, &id).await
        }
        Some(Cmd::Ls) => client_ls(load_client_config(target, port, ssh_port)?).await,
        Some(Cmd::Kill { id }) => {
            client_kill(load_client_config(target, port, ssh_port)?, &id).await
        }
        ...
        None => client_create(load_client_config(target, port, ssh_port)?).await,
    }
}

/// Client-side config: the file, with CLI overrides and the runtime SSH port.
///
...
fn load_client_config(
    ...
    let mut cfg = Config::load()?.with_client_overrides(target, port);
...
mod tests {
    ...
    #[test]
    fn ssh_port_is_global_and_validated() {
        for flag in ["-p", "--ssh-port"] {
            for operation in [
                vec![],
                vec!["ls"],
                vec!["attach", "abc"],
                ...
                for before in [true, false] {
                    let mut args = vec!["thther", "-t", "user@host", "--port", "62000"];
                    if before {
                        args.extend([flag, "2222"]);
                    }
                    ...
                    assert_eq!(cli.ssh_port.unwrap().get(), 2222);
                    assert_eq!(cli.port.unwrap().get(), 62000);
                    assert_eq!(cli.target.as_deref(), Some("user@host"));
                }
            }
            for port in ["0", "-1", "65536", "invalid", "1.5", ""] {
                assert!(
                    Cli::try_parse_from(["thther", flag, port]).is_err(),
                    "{flag} {port}"
                );
            }
            for port in ["1", "65535"] {
                let cli = Cli::try_parse_from(["thther", flag, port]).unwrap();
                assert_eq!(cli.ssh_port.unwrap().get().to_string(), port);
            }
            assert!(Cli::try_parse_from(["thther", flag]).is_err());
        }
        assert!(Cli::try_parse_from(["thther"]).unwrap().ssh_port.is_none());
    }
    ...
    fn port_is_global_and_validated() {
        ...
        for port in ["0", "-1", "65536", "invalid"] {
            assert!(
                Cli::try_parse_from(["thther", "--port", port]).is_err(),
                "{port}"
            );
        }
        for port in ["1", "65535"] {
